...
import { canPlaceItemAtAnchorIndex, normalizeOuterSlots } from '../domain/topLevelLayout'
...
interface ApplyOuterDropFromSessionParams {
  ...
  mode?: 'paged' | 'linear' | 'compact-page'
...
export const applyOuterDropFromSession = ({
  ...
  const baseSlots = resolveCommittedBaseSlots(
    mode === 'compact-page' ? session.workingOrder : (sourceSlots ?? session.workingOrder),
    dragIds
  )
  ...
  const candidateDropIndex =
    mode === 'compact-page'
      ? (session.previewSlotIndex ?? sourceFallbackIndex)
  ...
  if (mode === 'compact-page') {
    const nextItems = hadDraggedInBase ? base : [...base, session.draggingItem]
    return {
      items: nextItems,
      slots: applyCompactPageDrop({
        slots: nextSlots,
        items: nextItems,
...
const applyCompactPageDrop = ({
  ...
  const safeMinPageCount = Math.max(
    1,
    minPageCount ?? Math.ceil(Math.max(slots.length, safePageSize) / safePageSize)
  )
  const targetPage = clampNumber(
    Math.floor(Math.max(0, dropIndex) / safePageSize),
    0,
    safeMinPageCount - 1
  )
  ...
  while (nextSlots.length < safeMinPageCount * safePageSize) {
    nextSlots.push(null)
  }

  const targetPageIds = nextSlots
  ...
  const targetLocalIndex =
    targetCompactIndex >= 0
      ? targetCompactIndex
      : clampNumber(dropIndex - pageStart, 0, targetPageIds.length)
  const sourcePageIds = dragIds.filter(id => targetPageIds.includes(id))
  const foreignDragIds = dragIds.filter(id => !targetPageIds.includes(id))
  const reorderedIds =
    sourcePageIds.length > 0 && foreignDragIds.length === 0
      ? (() => {
          const leadId = dragIds[0]
          const remaining = targetPageIds.filter(id => id !== leadId)
          const insertIndex = clampNumber(targetLocalIndex, 0, remaining.length)
          remaining.splice(insertIndex, 0, leadId)
          return remaining
        })()
      : (() => {
          const insertIndex = clampNumber(targetLocalIndex, 0, targetPageIds.length)
          const next = [...targetPageIds]
          next.splice(insertIndex, 0, ...dragIds)
          return next
        })()

  for (let index = 0; index < nextSlots.length; index += 1) {
    if (nextSlots[index] && dragIdSet.has(nextSlots[index] as string)) {
  ...
  reorderedIds.slice(0, safePageSize).forEach((id, index) => {
    nextSlots[pageStart + index] = id
  })

  const overflowIds = reorderedIds.slice(safePageSize)
...
export const applyMultiOuterDropFromSession = ({
  ...
  const nextSlots = resolveCommittedBaseSlots(
    mode === 'compact-page' ? session.workingOrder : (sourceSlots ?? session.workingOrder),
    dragIds
  )
