...
| D-004 |        | 进行中 | 优化搜索结果面板顶部 选中态                  | 优化搜索结果面板顶部 全部、快捷入口、文件以及右侧排序这些选中态的样式，现在选中态样式太扁了 |
| D-005 |        | 待开发 | 优化添加分组弹窗的分组名称输入框的聚焦框样式 | 添加分组弹窗的分组名称输入框的聚焦框样式和其他输入框不一致，没有用同一个组件？ |
| D-006 |        | 待开发 | 自定义背景 + 主题色                          | 添加主题色功能，用户可以自行选择主题色，以及开发自定义背景，还能从背景自动提取合适的主题色 |
| D-007 |        | 待开发 | 修复拖动文件到遮罩层文件夹没有关闭的问题     | 拖动图标到文件夹，打开文件夹后文件夹拖动图标进入文件夹中又拖到遮罩层，此时文件夹没有关闭，这应该是之前拆分模块代码弄出的问题，修复它 |
| D-007 |        | 待开发 | 优化搜索面板最佳快捷入口                     | 优化搜索面板最佳快捷入口，应该能和下方列表一样，可以通过上下左右键操控，回车打开 |
| D-008 |        | 待开发 | 优化图标库中 “移出图标库”按钮                | 优化图标库中 “移出图标库”按钮，这里在收窄的时候 库子会在按钮里换行 |
...
- 待用户确认：在浅色、深色和透明窗口样式下检查顶部选中态层次与排序菜单当前项的实际观感。
