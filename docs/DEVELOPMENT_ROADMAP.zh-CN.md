...
| D-002 | P0     | 进行中 | ESLint 告警清零与超大文件模块化 | 将当前 34 条 ESLint 告警分批降至 0；逐步把 11 个超预算手写文件拆回清晰的视图、控制器、领域和服务边界。 |
...
- `pnpm lint` 限定检查 `src`、`vite.config.ts` 和 `eslint.config.js`，当前为 `0 errors / 34 warnings`。
...
- 当前 34 条告警分布为：`exhaustive-deps` 18 条、`max-lines` 11 条、`set-state-in-effect` 4 条、`refs` 1 条；`only-export-components` 与 `immutability` 已清零。
- E-03 已将功能代码告警从 52 降到 23；当前总数额外包含后续启用的 11 条 1000 行 `max-lines` 门禁告警。
...
4. 完成 `E-04` 与 `M-03`。滚动网格复用分页网格已验证的布局尺寸、自动翻页和会话装配边界，不并行维护两套同义规则。
