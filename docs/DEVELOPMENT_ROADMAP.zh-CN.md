...
| D-007 | P0     | 进行中 | 修复拖动文件到遮罩层文件夹没有关闭的问题     | 拖动图标到文件夹并自动打开文件夹后，将图标移入文件夹再拖到遮罩层时，文件夹应自动关闭并继续顶层拖放。 |
...
2. 待用户确认：在滚动网格中复现“顶层图标自动打开文件夹 -> 移入内容区 -> 移到遮罩层”，确认文件夹关闭且拖拽继续。
3. 待用户确认：分页网格、从文件夹内部拖出、重新进入内容区和 Dock 落点行为没有回归。
