...
| D-003 | P1     | 待开发 | Rust 静态检查与超大模块治理     | 建立 rustfmt、Clippy、check、test 门禁，并将 9 个超预算 Rust 模块分批拆回 500 行预算。     |
...
截至 2026-07-28，发布记录曾执行 `cargo check`，但仓库尚未形成与前端 ESLint 等价的持续 Rust 质量门禁：

- 仓库未提供 `rustfmt.toml`、`clippy.toml` 或统一的 Rust 检查脚本。
- `package.json` 和 GitHub Actions 尚未执行 `cargo fmt --check`、`cargo clippy`、`cargo check` 与 `cargo test`。
- 当前文件预算脚本以 500 行作为手写 Rust 模块阈值，全量扫描仍有 9 个文件超出预算。
...
| `src-tauri/src/lib.rs`                                |     1549 | Tauri 初始化、状态装配、窗口事件、命令注册和业务实现混合                 | 仅保留应用装配与命令注册，将窗口、托盘、生命周期和业务操作迁入独立模块                    |
| `src-tauri/src/everything/ipc.rs`                     |     1139 | Everything IPC 连接、查询协议、结果转换和错误处理聚合                    | 拆分连接会话、协议编解码、查询执行、结果映射和错误契约                                    |
| `src-tauri/src/agent/llm.rs`                          |      798 | LLM 请求构建、流式响应、工具调用与供应商兼容逻辑耦合                     | 拆分请求模型、流式解码、工具调用编排和供应商适配器                                        |
| `src-tauri/src/icons/catalog_windows/operations.rs`   |      752 | Windows 图标目录扫描、提取、转换和持久化操作集中                         | 拆分扫描、提取、图像规范化、目录写入和失败恢复                                            |
| `src-tauri/src/icons/catalog_windows.rs`              |      725 | Windows 图标目录领域类型、查询、缓存和操作入口混合                       | 保留目录 facade，将缓存、查询策略和平台操作委派给子模块                                   |
| `src-tauri/src/ai.rs`                                 |      555 | AI 命令入口、请求编排、结果校验和运行记录边界不清晰                       | 命令处理器仅校验和转换输入，执行流程进入 application service，纯校验进入 domain           |
| `src-tauri/src/commands.rs`                           |      550 | 多领域 Tauri 命令集中，容易继续形成中央命令文件                           | 按窗口、图标、布局、搜索和设置领域拆分命令模块，中央文件只负责统一导出                     |
| `src-tauri/src/everything/runtime.rs`                 |      548 | 运行时启动、进程探测、路径解析、重试和状态转换集中                         | 拆分安装发现、进程生命周期、重试策略和运行态状态机                                        |
...
| 批次 | 状态   | 范围                         | 处理策略                                                                                                         | 完成后目标                         |
| ---- | ------ | ---------------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| R-00 | 待开发 | Rust 静态检查基线            | 增加统一脚本与 CI 门禁，执行 rustfmt、Clippy、check、test 和文件预算检查，并清理首次扫描发现的 Clippy 告警。     | 可重复执行的 Rust 零告警质量门禁  |
| R-01 | 待开发 | AI 与 LLM                    | 拆分 `agent/llm.rs` 和 `ai.rs` 的请求模型、流式解析、工具编排、结果校验与运行记录边界。                           | 2 个模块进入 500 行预算            |
